...
type api_hasher func(dest []byte, data []byte)

// makeHasher creates a repetitive hasher, allowing the same hash data structures to
// be reused between hash runs instead of requiring new ones to be created. The returned
// function is not thread safe!
func (api *PublicDebugAPI)makeHasher(h hash.Hash) api_hasher {
...
const (	
	api_epochLength        = 30000   // Blocks per epoch
)

// seedHash is the seed to use for generating a verification cache and the mining
// dataset.
func (api *PublicDebugAPI)seedHash(block uint64) []byte {
	seed := make([]byte, 32)
	if block < api_epochLength {
		return seed
	}
	keccak256 := api.makeHasher(sha3.NewLegacyKeccak256())
	for i := 0; i < int(block/api_epochLength); i++ {
		keccak256(seed, seed)
	}
	return seed
}
...
//		return "", fmt.Errorf("block #%d not found", number)
//	}
//	return fmt.Sprintf("DT%x", ethash.SeedHash(number)), nil
//}
